...
router.post("/blogs", upload.single('myImage'), async function (req, res, next) {
  ...
  try {
    ...
    await conn.query("INSERT INTO images(blog_id, file_path) VALUES(?, ?);", [blogId, file.path.substr(6).replaceAll("\\", "/")])
    conn.commit()
    res.send("success!");
  } catch (error) {
    await conn.rollback();
...
router.get("/blogs/:id", function (req, res, next) {
  ...
  const promise2 = pool.query(`SELECT * FROM comments LEFT OUTER JOIN 
  images ON images.comment_id = comments.id WHERE comments.blog_id=? `, [
    req.params.id,
  ]);
...
router.put("/blogs/:id", upload.single('myImage'), async function (req, res, next) {
  ...
  try {
    const file = req.file;
    if (file) {
      await conn.query("UPDATE images SET file_path=? WHERE id=?", [file.path.replaceAll("\\", "/"), req.params.id])
    }
